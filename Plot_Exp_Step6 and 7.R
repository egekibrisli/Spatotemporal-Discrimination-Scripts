...
library(tidyr)
df_long <- gather(df, variable, value, -Groups, -SE_sd, -ME_sd, -p_value, -significance)
...
ggplot(df_long, aes(x = value, y = Groups, fill = variable)) +
  ...
  geom_errorbar(aes(xmin = value - ifelse(variable == "SE_mean", SE_sd, ME_sd),
                    xmax = value + ifelse(variable == "SE_mean", SE_sd, ME_sd)),
