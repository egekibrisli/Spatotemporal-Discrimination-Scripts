...
data <- read.csv("/Users/PC/Dropbox/eloras_rats/data/first_half_step7expfiltered.csv")
...
se_SE <- sapply(list(naro_lm, naro_ml, wide_lm, wide_ml), function(group) {
  se <- sd(group$SE, na.rm = TRUE) / sqrt(length(group$SE))
  return(se)
})
se_ME <- sapply(list(naro_lm, naro_ml, wide_lm, wide_ml), function(group) {
  se <- sd(group$ME, na.rm = TRUE) / sqrt(length(group$ME))
  return(se)
})
...
df_long <- gather(df, variable, value, -Groups, -SE_se, -ME_se, -p_value, -significance)
...
ggplot(df_long, aes(x = value, y = Groups, fill = variable)) +
  geom_bar(stat = "identity", position = "dodge", alpha = 0.8) +
  geom_errorbar(aes(xmin = value - ifelse(variable == "SE_mean", SE_se, ME_se),
                    xmax = value + ifelse(variable == "SE_mean", SE_se, ME_se)),
                position = position_dodge(width = 0.9), width = 0.2) +
  geom_vline(xintercept = c(1, 2), linetype = "dashed", color = "black") +  # Add dashed lines for x = 1 and x = 2
  labs(title = "ME and SE Latencies by Groups For Second Half Retiming Durations", x = "Latencies From Duration Start (s)", fill = "Variable") +
