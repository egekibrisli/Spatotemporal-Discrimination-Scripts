...
df <- read.csv("/Users/PC/Dropbox/eloras_rats/data/all_data_step6columned.csv", check.names = FALSE) %>%
...
ggplot(df, aes(x = response_type, y = X.response_lat., fill = accuracy)) +
  ...
  ggtitle("Response Time Latencies by Subgroup For Normal Durations") +
  theme_classic() +
